...
export class MarketComponent implements OnInit {
    ...
    this.combinedFilters = [
      {
        key: 'level',
        fn: (i: any) =>
          i.level >= +this.charFilters.minLevel &&
          i.level <= +this.charFilters.maxLevel,
      },
    ];
  }
  ...
  setMainFilter(index: string, $event: any) {
    this.filter[index] =
      index === 'nftType' ? $event.target?.value : +$event.target?.value;
    if (index === 'nftType') {
      this.mainHeaders = [];
      this.combinedFilters = [
        (i: any) =>
          this.filter.element >= 0
            ? i.traitNum === this.filter.element
            : i.traitNum >= 0,
        (i: any) =>
          +i.finalPrice >= this.filter.minPrice &&
          +i.finalPrice <= this.filter.maxPrice,
      ];
      if (this.filter[index] === 'characters') {
        ...
        this.combinedFilters = [
          ...this.combinedFilters,
          (i: any) =>
            +(i.level || 0) + 1 >= +this.charFilters.minLevel &&
            +(i.level || 0) + 1 <= +this.charFilters.maxLevel,
          (i: any) =>
            +(i.power || 0) >= +this.charFilters.minPower &&
            +(i.power || 0) <= +this.charFilters.maxPower,
          (i: any) =>
            +(i.rep || 0) >= +this.charFilters.minRep &&
            +(i.rep || 0) <= +this.charFilters.maxRep,
        ];
        ...
        this.combinedFilters = [
          ...this.combinedFilters,
          (i: any) =>
            +(i.stars || 0) + 1 >= +this.weaponFilters.minStars &&
            +(i.stars || 0) + 1 <= +this.weaponFilters.maxStars,
          (i: any) =>
            +i.bonusPower >= +this.weaponFilters.minPower &&
            +i.bonusPower <= +this.weaponFilters.maxPower,
        ];
        ...
        this.combinedFilters = [
          ...this.combinedFilters,
          (i: any) =>
            +(i.stars || 0) + 1 >= +this.shieldFilters.minStars &&
            +(i.stars || 0) + 1 <= +this.shieldFilters.maxStars,
        ];
      }
      this.loadList();
    } else {
  ...
  setFilter() {
    this.clientFilter.pages = Math.trunc(
      this.filteredListings.length / this.clientFilter.limit
    );
    ...
    this.displayedListings = _.chunk(
      _.orderBy(
        this.filteredListings,
        Object.keys(this.tableSorts),
        Object.values(this.tableSorts)
      ),
      this.clientFilter.limit
    );
  }
  ...
  async loadList() {
    ...
    const nftAddress = this.web3Service.getOtherContractAddress(
      this.filter.nftType
    );

    this.maxListing = +(
      await marketContract.getNumberOfListingsForToken(nftAddress)
    ).toString();
    const listings = await marketContract.getListingSlice(
      nftAddress,
      this.filter.offset,
      this.filter.limit
    );
    ...
    const listingIds = [
      ...listings[1].map((i: any) => this.utilService.bnToNumber(i)),
    ];

    const nftsInfo = await this.web3Service.multicall(
      this.web3Service.getBatchCallData(
        this.web3Service.abis[this.filter.nftType],
        this.web3Service.getOtherContractAddress(this.filter.nftType),
        listingIds.map((nftId: number) => ({
          name: 'get',
          params: [nftId],
        }))
      )
    ...
    if (this.filter.nftType === 'characters') {
      ...
      const nftsRep = await this.web3Service.multicall(
        this.web3Service.getBatchCallData(
          this.web3Service.abis[this.filter.nftType],
          this.web3Service.getOtherContractAddress(this.filter.nftType),
          listingIds.map((nftId: number) => ({
      ...
      const nftsPower = await this.web3Service.multicall(
        this.web3Service.getBatchCallData(
          this.web3Service.abis[this.filter.nftType],
          this.web3Service.getOtherContractAddress(this.filter.nftType),
          listingIds.map((nftId: number) => ({
      ...
      const nftsStamina = await this.web3Service.multicall(
        this.web3Service.getBatchCallData(
          this.web3Service.abis[this.filter.nftType],
          this.web3Service.getOtherContractAddress(this.filter.nftType),
          listingIds.map((nftId: number) => ({
    ...
    const nftPrices = await this.web3Service.multicall(
      this.web3Service.getBatchCallData(
        this.web3Service.abis['market'],
        this.web3Service.getConfigAddress('market'),
        listingIds.map((nftId: number) => ({
          name: 'getFinalPrice',
    ...
    const sellerStatus = await this.web3Service.multicall(
      this.web3Service.getBatchCallData(
        this.web3Service.abis['market'],
        this.web3Service.getConfigAddress('market'),
        listingIds.map((nftId: number, i: number) => ({
          name: 'isUserBanned',
          params: [listings[2][i]],
        }))
      )
    );
    const targetBuyers = await this.web3Service.multicall(
      this.web3Service.getBatchCallData(
        this.web3Service.abis['market'],
        this.web3Service.getConfigAddress('market'),
        listingIds.map((nftId: number) => ({
    ...
    this.listings = nftsInfo.map((nftInfo: any, i: number) => ({
      ...
      seller: listings[2][i],
      price: +this.utilService.formatNumber(
        this.utilService.fromEther(this.utilService.bnToNumber(listings[3][i]))
      ),
      finalPrice: +this.utilService.formatNumber(
        this.utilService.fromEther(this.utilService.bnToNumber(nftPrices[i]))
      ),
    }));
    this.filteredListings = [...this.getFilteredResults()];
    this.setFilter();
  ...
  getLink(id: number | string) {
    return `https://bazaar.market/buy/${
      bazaarNfts[this.filter.nftType]
    }?id=${id}`;
  }
