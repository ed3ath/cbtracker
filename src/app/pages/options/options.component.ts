...
export class OptionsComponent implements OnInit, ComponentCanDeactivate {
  ...
  setCurrency(event: any) {
    this.currencyService.setActiveCurrency(
      event.target ? event.target.value : this.configService.currency
    );
  }

  setChain(event: any) {
    this.web3Service.setActiveChain(
      event.target ? event.target.value : this.configService.chain
    );
  }

  setTimezone(event: any) {
    this.configService.timezone = event.target
      ? event.target.value
  ...
  setLanguage(event: any) {
    this.configService.language = event.target
      ? event.target.value
  ...
  setFightMultiplier(event: any) {
    ...
    this.configService.setFightMultiplier(
      event.target ? +event.target.value : this.configService.fightMultiplier
    );
  }
  ...
  async toggleNewsAlert() {
    ...
    if (
      this.configService.newsAlert &&
      this.notifService.getPermission() !== 'granted'
    ) {
  ...
  async toggleAccountAlert() {
    ...
    if (
      this.configService.accountAlert &&
      this.notifService.getPermission() !== 'granted'
    ) {
      this.notifService.requestPermission();
    }
      ...
      this.web3Service.getNetwork(rpcUrl).then(async (networkId: any) => {
        if (+networkId.toString() > 0) {
          if (+networkId.toString() === +this.web3Service.getNetworkId(chain)) {
            ...
            Swal.fire(
              '',
              `Mismatch network Id. RPC URL provided returns ${networkId}. Require: ${this.web3Service.getNetworkId(
                chain
              )}`,
              'error'
            );
              ...
              this.configService.configKeys.forEach((key) => {
                if (data[key]) {
                  localStorage.setItem(
                    key,
                    this.utilService.isArrayOrObject(data[key])
