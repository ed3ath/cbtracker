...
function reputationToTier(reputation) {
  switch (reputation) {
    case ReputationTier.PEASANT:
      return "Peasant";
    case ReputationTier.TRADESMAN:
      return "<span style='color: cyan'>Tradesman</span>";
    case ReputationTier.NOBLE:
      return "<span style='color: green'>Noble</span>";
    case ReputationTier.KNIGHT:
      return "<span style='color: orange'>Knight</span>";
    case ReputationTier.KING:
      return "<span style='color: gold'>King</span>";
    default:
      return "Peasant";
  }
}
...
function AdjustStatForTrait(statValue, statTrait, charTrait) {
  ...
  else if (statTrait === WeaponTrait.PWR) value = Math.floor(value * 1.03);
  return value;
}
...
function getWinChance(charData, weapData, enemyPower, enemyElement) {
  ...
  const playerMax = totalPower * totalMultiplier * 1.0;
  const enemyMin = enemyPower * 0.9;
  const enemyMax = enemyPower * 1.1;
  let win = 0;
  let lose = 0;
  for (
    let playerRoll = Math.floor(playerMin);
    playerRoll <= playerMax;
    playerRoll++
  ) {
    for (
      let enemyRoll = Math.floor(enemyMin);
      enemyRoll <= enemyMax;
      enemyRoll++
    ) {
...
function formatDurationToUnit(n, unitNames) {
  return `${n.toFixed(0)} ${
    n.toFixed(0) === "1" ? unitNames[0] : unitNames[1]
  }`;
}
